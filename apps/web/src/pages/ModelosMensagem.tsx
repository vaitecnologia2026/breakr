...
function ModalMidia({ aoFechar, aoInserir }: { aoFechar: () => void; aoInserir: (m: { url: string; mimetype: string; nome: string }) => void }) {
  ...
  // Ícones no mesmo padrão dos demais da tela (SVG de linha, stroke currentColor).
  const svgProps = { width: 26, height: 26, viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', strokeWidth: 2, strokeLinecap: 'round' as const, strokeLinejoin: 'round' as const };
  const OPCOES: { rotulo: string; accept: string; icone: ReactNode }[] = [
    {
      rotulo: 'Imagem', accept: 'image/*',
      icone: (<svg {...svgProps}><rect x="3" y="3" width="18" height="18" rx="2" ry="2" /><circle cx="8.5" cy="8.5" r="1.5" /><polyline points="21 15 16 10 5 21" /></svg>),
    },
    {
      rotulo: 'Vídeo', accept: 'video/*',
      icone: (<svg {...svgProps}><polygon points="23 7 16 12 23 17 23 7" /><rect x="1" y="5" width="15" height="14" rx="2" ry="2" /></svg>),
    },
    {
      rotulo: 'Áudio', accept: 'audio/*',
      icone: (<svg {...svgProps}><path d="M9 18V5l12-2v13" /><circle cx="6" cy="18" r="3" /><circle cx="18" cy="16" r="3" /></svg>),
    },
    {
      rotulo: 'Arquivo', accept: '.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.csv',
      icone: (<svg {...svgProps}><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48" /></svg>),
    },
  ];
        ...
        <div className="brk-modal-body">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {OPCOES.map((o) => (
              ...
              >
                <span style={{ display: 'inline-flex', color: 'var(--texto-suave)' }} aria-hidden="true">{o.icone}</span>
                {o.rotulo}
              </button>
            ))}
          </div>
